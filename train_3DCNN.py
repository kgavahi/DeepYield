...
def create_model():
	model = tf.keras.models.Sequential([
		tf.keras.layers.Conv3D(9, (1,3,3),padding='same',data_format='channels_last', activation='relu', input_shape=(24, image_shape[0], image_shape[1], 9)),
		...
		tf.keras.layers.MaxPooling3D((2, 2, 2),strides=(2, 2, 2),data_format='channels_last'),
		tf.keras.layers.Conv3D(256, (3,3,3),padding='same',data_format='channels_last', activation='relu'),

		tf.keras.layers.MaxPooling3D((2, 2, 2),strides=(2, 2, 2),data_format='channels_last'),
		tf.keras.layers.Conv3D(512, (3,3,3),padding='same',data_format='channels_last', activation='relu'),
...
print('epoch:', np.argmin(x)+1, 'RMSE:', np.sqrt(np.min(x)))
